 ...
 * @since 1.0
 */
class GravityFlow_Oauth1_Client {
	...
	public function __construct( $config, $connection_identifier, $url ) {
		...
		foreach ( $required_params as $param ) {
			if ( ! isset( $config[ $param ] ) ) {
				throw new InvalidArgumentException( sprintf( 'Missing OAuth1 client configuration: %s - see includes/class-gravityflow-oauth1-client.php', $param ) );
			}
		}
		$this->url = $url;
		$this->config = $config;
	...
	function get_auth_urls() {
		...
		$page = vip_safe_wp_remote_get( $url );

		if ( ! is_wp_error( $page ) ) {
			$headers = $page['headers'];
			$link_parts = explode( '; ',$headers['link'] );
			$this->api_base_url = str_replace( array( '<', '>' ), '', $link_parts[0] );
			$api_details = vip_safe_wp_remote_get( $this->api_base_url );
